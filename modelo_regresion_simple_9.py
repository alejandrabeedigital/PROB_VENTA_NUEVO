...
ARCHIVO_OUT = "todo_con_resultados_9.csv"
TARGET = "ganada"
...
# 👇 añadimos las nuevas categóricas
features_cat = [
    "ct_merclie",
    "excliente_cat",
...
if faltan:
    raise ValueError(f"Faltan columnas en el CSV de entrada: {faltan}")

df_model = df[cols_modelo].copy()

pos = int(df_model[TARGET].sum())
neg = int((df_model[TARGET] == 0).sum())
tasa_global = pos / (pos + neg) if (pos + neg) > 0 else np.nan
