...
ARCHIVO_OUT = "todo_con_resultados_8.csv"
TARGET = "ganada"
...
# --- asegurar tipos numéricos (igual que antes)
for col in features_num:
    df[col] = pd.to_numeric(df[col], errors="coerce")

# --- normalizar excliente (igual que antes)
df["excliente"] = df["excliente"].map(
    {True: "1", False: "0", "True": "1", "False": "0"}
).fillna(df["excliente"].astype(str))

# ✅ FIX MÍNIMO PARA QUE NO PETE:
#   Dejamos las categóricas como object "clásico"
for c in features_cat:
    if c in df.columns:
        df[c] = df[c].astype(object)

# =========================
# 3.1 DESCRIPTIVOS (TOTAL y %) tipo descriptivos_movil / descriptivos_ranking
# =========================
print("\n==============================")
print("  DESCRIPTIVOS (TOTAL y %)")
print("==============================\n")

def guardar_descriptivo_categorica(df_in: pd.DataFrame, col: str, out_name: str) -> None:
    # Incluye NaN como categoría "NaN" para que cuadre total/%
...
# Categóricas: 1 archivo por variable (total y %)
for c in features_cat:
    if c in df.columns:
        guardar_descriptivo_categorica(df, c, f"descriptivos_{c}.csv")

# Numéricas: bins por deciles para tener total y %
def guardar_descriptivo_numerica_deciles(df_in: pd.DataFrame, col: str, out_name: str) -> None:
    s = pd.to_numeric(df_in[col], errors="coerce")
    tmp = pd.DataFrame({col: s}).dropna()
    if tmp.empty:
        print(f"⚠️ {col}: sin datos numéricos para descriptivo.")
        return

    # qcut puede colapsar bins si hay muchos repetidos → duplicates='drop'
...
# =========================
# 5.1 DESCRIPTIVO TRAIN SET (ranking_cat x claim_business_cat)
# =========================
df_train_desc = X_train.copy()
df_train_desc["ganada"] = y_train.values

tabla_cruce = (
    df_train_desc
    .groupby(["ranking_cat"])["ganada"]
    .agg(
        observaciones="count",
        ganadas="sum"
    )
...
tabla_cruce["tasa_venta"] = tabla_cruce["ganadas"] / tabla_cruce["observaciones"]
tabla_cruce.to_csv("conteo_train_ranking_claim.csv", index=False)

print("\n--- Conteo TRAIN ranking_cat x claim_business_cat ---")
print(tabla_cruce)
